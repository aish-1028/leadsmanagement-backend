...
exports.bulkInsertLeads = exports.getLeadStats = exports.deleteLead = exports.updateLead = exports.getLeadById = exports.createLead = exports.getLeads = void 0;
...
const getLeads = async (req, res) => {
    try {
        const { page = 1, limit = 10, search = "", status = "all", counsellor = "all", sortBy = "createdAt", sortOrder = "desc", allUsers = false, } = req.query;
...
const createLead = async (req, res) => {
    try {
        const { name, email, status, phone, city, workExp, financeDomain, details, followUpDetails, counsellor, calledOn, visitingDate, visitDoneDate, counsellorWhoTookVisit, } = req.body;
        // Validation
        if (!name || !phone) {
            res.status(400).json({
                success: false,
        ...
        const existingLead = await Lead_1.default.findOne({ phone: phone.trim() });
        if (existingLead) {
            res.status(400).json({
                success: false,
...
const getLeadById = async (req, res) => {
    try {
        ...
        if (req.user && req.user.role !== "admin" && lead.createdBy.toString() !== req.user._id.toString()) {
...
const updateLead = async (req, res) => {
    try {
        const { id } = req.params;
        const { name, email, status, phone, city, workExp, financeDomain, details, followUpDetails, visitDoneDate, visitingDate, counsellorWhoTookVisit, counsellor, calledOn, } = req.body;
        const lead = await Lead_1.default.findById(id);
        if (!lead) {
            res.status(404).json({
                success: false,
...
exports.bulkInsertLeads = [
    ...
    async (req, res) => {
        try {
            ...
            const existingLeads = await Lead_1.default.find({ phone: { $in: phones } }).select('phone');
            ...
            if (leadsData.length > 0) {
                try {
                    const savedLeads = await Lead_1.default.insertMany(leadsData, { ordered: false });
